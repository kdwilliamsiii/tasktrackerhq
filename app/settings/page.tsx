  ...
  return <AppShell active="Settings" eyebrow="Manage" title="Settings" description="Manage your account, preferences, and integrations.">
    <div className="settings-grid">
      <section className="panel">
        ...
        <div className="account-summary">{session?.user?.image ? <><img className="account-avatar account-avatar-image" src={session.user.image} alt="" /></> : <span className="account-avatar">{session?.user?.name?.slice(0, 1).toUpperCase() || "?"}</span>}<div><strong>{status === "loading" ? "Checking session..." : session?.user?.name || "Guest user"}</strong><small>{session?.user?.email || "Not signed in"}</small></div>{session && <span className={`role-badge role-${session.user.role}`}>{session.user.role}</span>}</div>
