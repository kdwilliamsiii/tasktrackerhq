...
export default function QuickAddWidget() {
    ...
    try {
      if (type === "task") {
        const response = await fetch("/api/tasks", {
          ...
          body: JSON.stringify({ title: value.trim(), priority: "Medium", category: "Quick add" }),
        });
        ...
        saved.unshift({ id: crypto.randomUUID(), text: value.trim(), createdAt: new Date().toISOString() });
        localStorage.setItem(key, JSON.stringify(saved.slice(0, 50)));
        window.dispatchEvent(new Event("tasktracker-quick-add"));
      }
      setValue("");
      setMessage(`${type[0].toUpperCase()}${type.slice(1)} added`);
    } catch {
  ...
  return (
    <section className="quick-add-widget" aria-labelledby="quick-add-title">
      <div className="quick-add-heading"><div><span className="dashboard-kicker">QUICK ADD</span><h2 id="quick-add-title">Capture an idea</h2></div><span className="quick-add-plus">+</span></div>
      <form onSubmit={submit} className="quick-add-form">
        ...
        <button type="submit" aria-label={`Add ${type}`} disabled={saving || !value.trim()}>+</button>
      </form>
