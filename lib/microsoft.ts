...
export async function syncMicrosoftCalendar(accessToken?: string) {
  ...
  return { provider: "microsoft", synced: true, events: (data.value || []).map((event: { id: string; subject?: string; start?: { dateTime?: string } }) => ({ id: event.id, title: event.subject || "Untitled event", date: event.start?.dateTime })) };
}
