...
export async function syncGoogleCalendar(accessToken?: string) {
  ...
  return { provider: "google", synced: true, events: (data.items || []).map((event: { id: string; summary?: string; start?: { dateTime?: string; date?: string } }) => ({ id: event.id, title: event.summary || "Untitled event", date: event.start?.dateTime || event.start?.date })) };
}
